...
EARLIEST = dt.date(2020, 1, 5)  # First complete Sunday-to-Saturday week of 2020.
STROKES = 8
PATTERNS = {
    'heart': ['.##.##.', '#######', '#######', '.#####.', '..###..', '...#...', '.......'],
    'cat': ['#.....#', '##...##', '#.###.#', '#.#.#.#', '#..#..#', '.#...#.', '..###..'],
...
def main():
    ...
    parser.add_argument('--backfill', action='store_true',
                        help='Extend art through June 1, 2024 without changing the daily cursor.')
    ...
    cursor = state.get('backfill', {}) if args.backfill else state
    last = dt.date.fromisoformat(cursor['date']) if cursor else (EARLIEST if args.backfill else START)
    through = min(args.through, START - dt.timedelta(days=1)) if args.backfill else args.through
    count = 0
    day = last
    while day <= through:
        name = pixel(day)
        done = cursor.get('stroke', 0) if cursor.get('date') == str(day) else 0
        if name:
            for stroke in range(done + 1, STROKES + 1):
                count += 1
                if args.dry_run:
                    continue
                cursor = {'date': str(day), 'symbol': name, 'stroke': stroke}
                if args.backfill:
                    state['backfill'] = cursor
                else:
                    state.update(cursor)
                state['purpose'] = 'Intentional decorative contribution art'
    ...
    print(f'{"Would create" if args.dry_run else "Created"} {count} art commits through {through}.')
