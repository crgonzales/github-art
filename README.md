...
## Extend the historical art

```sh
python3 art.py --backfill --dry-run
python3 art.py --backfill
git push origin master
```

The historical extension starts with the first full week of 2020 (January 5), continues the same symbol cycle backward from June 2024, and keeps its own progress inside the single state file. Reruns are safe and leave the daily cursor intact.
